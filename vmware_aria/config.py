"""Configuration management for VMware Aria Operations.
...
CONFIG_DIR = Path.home() / ".vmware-aria"
CONFIG_FILE = CONFIG_DIR / "config.yaml"
ENV_FILE = CONFIG_DIR / ".env"

_log = logging.getLogger("vmware-aria.config")
...
class TargetConfig:
    """An Aria Operations connection target."""
    ...
    def get_password(self, target_name: str) -> str:
        ...
        Convention: VMWARE_ARIA_<TARGET>_PASSWORD
        where <TARGET> is upper-cased with hyphens replaced by underscores.
        """
        env_key = f"VMWARE_ARIA_{target_name.upper().replace('-', '_')}_PASSWORD"
...
def load_config(config_path: Path | None = None) -> AppConfig:
    ...
    env_override = os.environ.get("VMWARE_ARIA_CONFIG")
    ...
    for name, t in raw.get("targets", {}).items():
        targets[name] = TargetConfig(
            ...
            auth_source=t.get("auth_source", "LOCAL"),
        )
